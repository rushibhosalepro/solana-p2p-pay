...
const QRScanner = ({ onResult }: { onResult: (data: string) => void }) => {
  ...
  return (
    <div className="w-full h-full relative flex flex-col items-center justify-center">
      ...
      <div className="absolute bottom-4 flex flex-col items-center gap-2">
        {error && (
          <p className="text-red-500 text-sm bg-white px-3 py-1 rounded">
            {error}
          </p>
        )}
        <label className="bg-white px-4 py-2 text-sm rounded-lg shadow cursor-pointer hover:bg-gray-100 transition">
...
const QRCodePage = () => {
  const handleScanResult = (data: string) => {
    const solanaPayUrl = data.trim();
    const [recipientWithSlash, queryString] = solanaPayUrl
      .replace("solana:", "")
      .split("?");
    const redirectUrl = `/send/confirm/${recipientWithSlash}${queryString ? "?" + queryString : ""}`;
    window.location.href = redirectUrl;
  };

  return (
    <main className="min-h-screen h-screen bg-gray-100 flex items-center justify-center p-6">
      <div className="relative w-96 h-full bg-white rounded-[1.2rem] shadow-2xl overflow-hidden flex flex-col">
        <div className="absolute top-0 left-0 right-0 z-50 bg-gradient-to-b from-black/60 to-transparent p-6 flex items-center justify-between">
          <h1 className="text-white text-xl font-bold">Scan QR Code</h1>
          <Link
            href="/qr/generate"
            className="bg-white p-2.5 rounded-xl hover:bg-gray-100 transition-all shadow-lg"
          >
            <svg
